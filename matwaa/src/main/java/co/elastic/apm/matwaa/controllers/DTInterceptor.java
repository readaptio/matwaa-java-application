 ...
 * from the environment variable matwaa_SERVICES
 * The probability to delegate the call is obtained from the environment variable matwaa_DT_PROBABILITY
...
public class DTInterceptor extends HandlerInterceptorAdapter {
    ...
    public DTInterceptor(Environment env) {
        ...
        hostList = env.getProperty("matwaa_SERVICES", "").split(",");
        hostList = Arrays.stream(hostList).filter(s -> !s.equals("")).toArray(String[]::new);
        try {
            dtProb = Float.parseFloat(env.getProperty("matwaa_DT_PROBABILITY", "0.5"));
