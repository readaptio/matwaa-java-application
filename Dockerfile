...
WORKDIR /usr/src/java-code/matwaa

#Bring the latest frontend code
COPY --from=matwaa/matwaa-frontend:latest /app/build src/main/resources/public

RUN mvn -q -B package -DskipTests
RUN cp -v /usr/src/java-code/matwaa/target/*.jar /usr/src/java-app/app.jar
...
CMD java -javaagent:/app/elastic-apm-agent.jar -Dspring.profiles.active=${matwaa_JAVA_PROFILE:-}\
                                        -Dserver.port=${matwaa_SERVER_PORT:-}\
                                        -Dserver.address=${matwaa_SERVER_ADDRESS:-0.0.0.0}\
